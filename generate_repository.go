...
func writeFileRepository(mystruct interface{}) (string, error) {
	...
	_, err = file.WriteString(fmt.Sprintf("Save%s(%s %s) (%s %s)%s", name, strings.ToLower(name), "entity."+name, "string,", "error", "\n"))
	if isError(err) {
		return "", err
	}
	_, err = file.WriteString(fmt.Sprintf("Update%s(%s %s) (%s %s)%s", name, strings.ToLower(name), "entity."+name, "string,", "error", "\n"))
	if isError(err) {
		return "", err
	}
	...
	_, err = file.WriteString(fmt.Sprintf("//Generated by Micagen at %v", time.Now()))
	if isError(err) {
		return "", err
	}
	// Save file changes.
